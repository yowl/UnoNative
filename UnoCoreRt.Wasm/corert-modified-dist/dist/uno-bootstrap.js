            ...
            App.fetchWithProgress(
                ...
                .then(response => {
                    if (Module.isElectron()) {
                        ...
                        return createWasm.instantiateAsync();
                    }
                });
        }
        else {
...
var MonoRuntime = {
    ...
    init: function () {
        ...
        // TODO: why is this a problem and the previous 4 are ok?
...
//        this.mono_string_get_utf8 = Module.cwrap("mono_wasm_string_get_utf8", "number", ["number"]);
  ...
  //      this.mono_wasm_obj_array_new = Module.cwrap("mono_wasm_obj_array_new", "number", ["number"]);
    ...
    conv_string: function (mono_obj) {
        if (mono_obj === 0)
            return null;
        //TODO: what is this call for, to stop it being collected?  Why not just have the runtime marshal it??
        //const raw = this.mono_string_get_utf8(mono_obj);
        const res = Module.UTF8ToString(mono_obj);
        console.log('conv string is ' + res);
        //TODO: see above
//        Module._free(raw);
...
var App = {
    ...
    mainInit: function () {
        ...
        // TODO: going to need this I guess
...
//                just call the wasm _main
   var entryFunction = Module['_main'];


                // TODO: where to get the args?
  var args = args || [];
  ...
  try {
    ...
    getFetchInit: function (url) {
        ...
        if (config.files_integrity && config.files_integrity.hasOwnProperty(fileName)) {
...
console.log(`process dependency (${dependency}) on emscripten`);
...
console.log(`process dependency (${dependency}) on no empscripten`);
                        checkDone(dependency);
                    }
                };
